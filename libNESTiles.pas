...
Procedure DrawPatterns(var Bitmap: TBitmap; var NES: TNESFile; CHROffset: Longint);
Procedure DrawTilemap(var Bitmap: TBitmap; var NES: TNESFile; CHROffset,PRGOffset: Integer; Sx,Sy: Integer);
Procedure DrawSelection(var Bitmap: TBitmap; Selection: TRect);
Procedure DrawGrid(var Bitmap: TBitmap; A,B: Integer);
Procedure DrawUsedTiles(var Bitmap: TBitmap; Ofs, Sx: Integer; Area: TRect; var NES: TNESFile);
...
Procedure DrawTileLine(Buf: Pointer; TileLineA, TileLineB: Byte); assembler; stdcall;
 asm
     push     edi
     push     ecx
     mov      edi, Buf
           ...
           begin
                for j:=0 to 15 do
                 if((Offset+j) < (Longint(CHRData)+CHRSize)) then
                   ...
                   CurTile[j] := 0;
               case DrawMode of
                DRAW_NORMAL:    DrawTile(Bitmap, (i mod 16) shl 3, (i shr 4) shl 3, CurTile);
                DRAW_8X16:      DrawTile(Bitmap, ((i mod 32) shr 1) shl 3, ((i mod 2) shl 3) + ((i shr 5) shl 4), CurTile); // DONE
                DRAW_VERTICAL:  DrawTile(Bitmap, (i shr 4) shl 3, (i mod 16) shl 3, CurTile);
               end;
               Inc(Offset,16);
           end;
      end;
     ...
     if PRGData<>nil then
      begin
          ...
          for i:=0 to Sy-1 do
           for j:=0 to Sx-1 do
            begin
                if PRGOffset+Offset<PRGSize
                 then
                  begin
                      Tile:=Byte(Ptr(Longint(PRGData)+PRGOffset+Offset)^) shl 4;
                      DrawTile(Bitmap, j shl 3, i shl 3, TTile(Ptr(Longint(CHRData)+CHROffset+Tile)^))
                  end
                 else
                  begin
                      Blank.Left:=j shl 4;
                    ...
                    case DrawMode of
                     ...
                     DRAW_8X16:   // DONE
                      begin
                          ...
                          TileRect.Top:=1+(((CurTile mod 2) shl 4) + ((CurTile shr 5) shl 5));
                      end;
                    end;
...
begin
    with NES do
     begin
         if PRGData<>nil then FreeMem(PRGData,PRGSize);
         if CHRData<>nil then FreeMem(CHRData,CHRSize);
     end;
end;
